...
def h(node, end, G):
    ...
    lat2 = G.nodes[end]['lat']
    lon2 = G.nodes[end]['lon']
    return haversine(lat1, lon1, lat2, lon2) / 44.4

def astar(G, ini, end, initial_time, criterion, target_parent_station=None):
    visited = set()
    distance = {}
    previous = {}
    end_trip = None
    ...
    if criterion =='t':
        pq = [(initial_time + h(ini, end, G), initial_time, ini)]
    ...
    while pq:
        if criterion == 't':
            f, current_time, current = heappop(pq)
                ...
                if not edge[2]['isTransfer']:
                    if edge[2]['departure'] >= current_time:
                        if edge[2]['arrival'] < distance[neighbor]:
                            ...
                            heappush(pq, (edge[2]['arrival'] + h(neighbor, end, G), edge[2]['arrival'], neighbor))
                else:
                    if current_time < distance[neighbor]:
                        distance[neighbor] = current_time
                        previous[neighbor] = (current, None, current_time, None)
                        heappush(pq, (current_time + h(neighbor, end, G), current_time, neighbor))
        else:
                ...
                if not edge[2]['isTransfer']:
                    if edge[2]['departure'] >= current_time:
                        ...
                        if new_cost < distance[neighbor]:
                            ...
                            previous[(neighbor, edge[2]['trip_id'])] = (
                            current, current_trip, edge[2]['departure'], edge[2]['arrival'], edge[2]['trip_id'])
